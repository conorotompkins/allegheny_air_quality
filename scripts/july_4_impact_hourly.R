...
df <- read_csv("data/allegheny_aq_hourly.csv") %>% 
...
df_analysis <- df %>% 
  ...
  select(datetime, year, yday, month, mday, hour, site,
         #bcstat, h2s,
         no, no2, nox, noy, noydif,
         #out_rh, out_t, ozone, ozone2,
         pm10, pm10b, pm25, `pm25(2)`, pm25b, pm25t) %>% 
  select(datetime:site, no2:pm25t) %>% 
  tidyr::pivot_longer(-c(datetime, year, yday, month, mday, hour, site), names_to = "measure", values_to = "value") %>% 
...
df_analysis %>% 
  filter(!is.na(value)) %>% 
  ggplot(aes(hour, value, color = site)) +
  geom_point(alpha = .1) +
